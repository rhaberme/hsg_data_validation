...
def test_jumps():
    ...
    isjump = check_jumps(df_data, window=2, threshold=1, value_col_name=value_col_name)
    true_correct = all(isjump["Error"].iloc[[*range(39, 43), *range(58, 62), *range(104, 108)]])
    false_correct = not any(isjump["Error"].iloc[[*range(0, 39), *range(43, 58), *range(62, 104), *range(108, 126)]])

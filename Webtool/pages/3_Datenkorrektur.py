...
if check_gap:
    ...
    with exp_:
        st.session_state['custom_missing_values'] = st.text_input("Custom Missing Value")
...
if do_plausibility_checks:
    with st.spinner("Datenprüfung wird ausgeführt"):
        try:
            ...
            if check_gap:
                implausible_dates = dplau.check_gaps(df_raw, custom_missing_values=st.session_state["custom_missing_values"])
                tab1.markdown(f"Plausibility test gap: {implausible_dates["Missing"].sum()} inplausible data found.")
